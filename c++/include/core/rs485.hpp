...
class RS485 {
    ...
    BaudRate _bps;  // 波特率
    //std::string _en_gpio_chip; /*gpiochip338*/
    std::string _path_gpio_chip; /*gpiochip338*/
    ...
    int start() {
        ...
        /*按名称打开一个gpiochip。正确，返回值为芯片句柄，发生错误，返回NULL.
        std::string.c_str():将字符串的内容转换为c风格、以空字符结束的字符串。返回值为const char* */
        _gpio_chip = gpiod_chip_open(_path_gpio_chip.c_str());
        if (!_gpio_chip) {
            printf("Serial port: Fail to open %s!\n", _path_gpio_chip.c_str());
            return -1;
        }
        /*获取给定偏移量的GPIO行句柄。正确，返回指向GPIO行句柄的指针，如果发生错误，则返回NULL。*/
    ...
    int recv(unsigned char &charBuffer, size_t msTimeout = 0) {
        /*读取单个GPIO线的当前值。正确返回0或者1，错误返回-1并设置最后一个错误号。*/
        int value = gpiod_line_get_value(_gpio_line);
        if (value != 0) {
            /*设置单个GPIO的值。成功返回0，错误返回-1并设置最后一个错误号。*/
